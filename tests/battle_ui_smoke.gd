...
func _ready()->void:
	...
	for die_index in range(expected_sorted_values.size()):
		...
		assert(battle._physical_dice_board._detect_physical_top_value(physical_die)==expected_sorted_values[die_index])
		assert(str(physical_die.get_meta("settle_phase", ""))=="settled")
		assert(physical_die.freeze)
		assert((physical_die.get_child(0) as CollisionShape3D).shape is BoxShape3D)
