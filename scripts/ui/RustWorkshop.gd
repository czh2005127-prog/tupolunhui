...
func _get_level_descs(card_id: String) -> Array:
	match card_id:
		"jack_crt": return ["看2颗骰子", "看2颗且可用2次", "看3颗、2次且公开"]
		"rust_warrior": return ["给2名存活者各+1颗①", "追加对手-1骰", "所有存活者+1颗①"]
		"battery_kid": return ["免死2次", "免死3次", "免死3次且每次+1骰"]
		"signal_noise": return ["禁用玩家2个道具", "禁用玩家3个道具", "禁用全部道具"]
		"cyclops_lcd": return ["每局删2个点数", "每局删3个点数", "删3个且暴露1颗普通骰"]
		"chamberlain": return ["开局获得5个道具", "5道具且保底1稀有", "6道具且保底2稀有"]
		"recycler": return ["有人质疑失败时+2骰", "+2骰且吸被质疑者1骰", "每轮结束再+1骰"]
		"lucky_one": return ["每轮保证2个①", "每轮保证3个①", "其①仅自己可作万能"]
		"referee": return ["每局强制行动1次", "每局强制行动2次", "每局强制行动3次"]
		"table_ghost": return ["淘汰后附身2人", "附身者1轮免疫质疑"]
		"alliance_oled": return ["知道n-1人的全部骰子"]
		"casino_owner": return ["每人+2颗暗骰", "+2暗骰且自己能看", "每人+3暗骰且自己全看到"]
		"prophet": return ["重掷后+1骰", "每轮可重掷2次"]
	return ["—"]
